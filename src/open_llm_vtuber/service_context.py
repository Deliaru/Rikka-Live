...
from .rikka.persona import build_rikka_system_prompt
...
class ServiceContext:
    ...
    def __str__(self):
        return (
            ...
            f"    Details: {redact_debug_text(json.dumps(self.system_config.model_dump(), indent=6)) if self.system_config else 'None'}\n"
            ...
            f"    Config: {redact_debug_text(json.dumps(self.character_config.asr_config.model_dump(), indent=6)) if self.character_config.asr_config else 'None'}\n"
            f"  TTS Engine: {type(self.tts_engine).__name__ if self.tts_engine else 'Not Loaded'}\n"
            f"    Config: {redact_debug_text(json.dumps(self.character_config.tts_config.model_dump(), indent=6)) if self.character_config.tts_config else 'None'}\n"
            f"  LLM Engine: {type(self.agent_engine).__name__ if self.agent_engine else 'Not Loaded'}\n"
            f"    Agent Config: {redact_debug_text(json.dumps(self.character_config.agent_config.model_dump(), indent=6)) if self.character_config.agent_config else 'None'}\n"
            f"  VAD Engine: {type(self.vad_engine).__name__ if self.vad_engine else 'Not Loaded'}\n"
            f"    Agent Config: {redact_debug_text(json.dumps(self.character_config.vad_config.model_dump(), indent=6)) if self.character_config.vad_config else 'None'}\n"
            f"  System Prompt: {redact_debug_text(self.system_prompt) if self.system_prompt else 'Not Set'}\n"
    ...
    async def load_cache(
        ...
        logger.debug(
            ...
            f"{redact_debug_text(character_config.model_dump())}"
        )
    ...
    def init_asr(self, asr_config: ASRConfig) -> None:
        if not self.asr_engine or (self.character_config.asr_config != asr_config):
            ...
            if asr_model is None or asr_model == "none":
                logger.info("ASR is disabled.")
                asr_kwargs = {}
            else:
                logger.info(f"Initializing ASR: {asr_model}")
                asr_kwargs = getattr(asr_config, asr_model).model_dump()
    ...
    async def init_agent(self, agent_config: AgentConfig, persona_prompt: str) -> None:
        ...
        try:
            ...
            logger.debug(
                ...
                f"length={len(system_prompt)}, preview={redact_debug_text(system_prompt)}"
            )
    ...
    async def construct_system_prompt(self, persona_prompt: str) -> str:
        ...
        """
        if persona_prompt.strip() in {"$rikka_persona", "__RIKKA_PERSONA__"}:
            persona_prompt = build_rikka_system_prompt(
                getattr(self.live2d_model, "model_info", None)
            )

        logger.debug(
            "Constructing persona prompt: "
            f"length={len(persona_prompt)}, preview={redact_debug_text(persona_prompt)}"
        )
        ...
        try:
            ...
            if new_character_config_data:
                ...
                logger.debug(
                    ...
                    f"{redact_debug_text(self.character_config.model_dump())}"
                )
