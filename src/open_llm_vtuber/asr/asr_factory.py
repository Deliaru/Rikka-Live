...
class ASRFactory:
    @staticmethod
    def get_asr_system(system_name: str | None, **kwargs) -> ASRInterface:
        if system_name is None or system_name in {"none", "disabled", "no_asr"}:
            from .null_asr import VoiceRecognition as NullASR

            return NullASR()
        elif system_name == "faster_whisper":
