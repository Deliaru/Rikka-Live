...
from .rikka.routes import init_rikka_routes
...
class WebSocketServer:
    ...
    def __init__(self, config: Config, default_context_cache: ServiceContext = None):
        ...
        # Project-owned transparent co-streaming overlay.
        self.app.mount(
            "/overlay",
            CORSStaticFiles(directory="overlay", html=True),
            name="overlay",
        )
    ...
    @staticmethod
    def clean_cache():
        """Clean transient cache files without deleting local Rikka state."""
        cache_dir = "cache"
        ...
        for name in os.listdir(cache_dir):
            if name in PERSISTENT_CACHE_FILES:
                continue
