...
class OpenAICompatibleConfig(StatelessLLMBaseConfig):
    ...
    project_id: str | None = Field(None, alias="project_id")
    temperature: float = Field(1.0, alias="temperature")
    timeout_seconds: float = Field(60.0, gt=0, alias="timeout_seconds")
    ...
    _OPENAI_COMPATIBLE_DESCRIPTIONS: ClassVar[dict[str, Description]] = {
        ...
        "api_mode": Description(
            en="OpenAI-compatible endpoint family: chat completions or responses",
            zh="OpenAI 兼容端点类型：Chat Completions 或 Responses",
        ),
        "temperature": Description(
            en="What sampling temperature to use, between 0 and 2.",
            zh="使用的采样温度，介于 0 和 2 之间。",
        ...
        "timeout_seconds": Description(
            en="Maximum seconds to wait for the LLM request or any streamed chunk",
            zh="LLM 请求或任一流式分片的最大等待秒数",
        ),
    }
