...
class SherpaOnnxASRConfig(I18nMixin):
    ...
    streaming: Optional[Dict[str, Any]] = Field(None, alias="streaming")
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "streaming": Description(
            en="Dedicated Sherpa ONNX streaming ASR model configuration",
            zh="Sherpa ONNX 流式 ASR 专用模型配置",
        ),
    }
...
class ASRConfig(I18nMixin):
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "asr_model": Description(
            en="Speech-to-text model to use; null or 'none' disables ASR",
            zh="要使用的语音识别模型；null 或 'none' 表示禁用 ASR",
        ),
    ...
    def check_asr_config(cls, values: "ASRConfig", info: ValidationInfo):
        ...
        selected_config = getattr(values, asr_model, None)
        if selected_config is None:
            raise ValueError(f"Missing config block for selected ASR model: {asr_model}")

        # Only validate the selected ASR model
        selected_config.model_validate(selected_config.model_dump())
