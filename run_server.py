...
class PointerAccessLogFilter(logging.Filter):
    """Suppress noisy successful global-pointer polling access logs."""

    POINTER_PATH = "/rikka/overlay/pointer"
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        try:
            ...
            status = None
        if self.POINTER_PATH in path and (status is None or status < 400):
            return False
        return True


def install_access_log_filters() -> None:
    logging.getLogger("uvicorn.access").addFilter(PointerAccessLogFilter())
